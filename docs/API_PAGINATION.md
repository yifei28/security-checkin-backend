...
|------|------|------|
| `name` | string | 姓名模糊搜索 |
| `siteId` | string | 单位ID，如 `site_1` 或 `1` |
| `employmentStatus` | string | 在职状态：`ACTIVE` / `PROBATION` / `RESIGNED` / `TERMINATED` / `all` |
| `role` | string | 角色：`TEAM_LEADER` / `TEAM_MEMBER` / `all` |

**请求示例：**
```
GET /api/guards?page=1&pageSize=10&sortBy=name&sortOrder=asc&name=张&siteId=site_1&employmentStatus=ACTIVE
```
...
**请求示例：**
```
GET /api/sites?page=1&pageSize=10&name=北京
```
...
- **2025-12-31**: 新增 Guards 和 Sites 后端筛选功能（name, siteId, employmentStatus, role）
